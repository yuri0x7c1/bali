...
import stylesCss from 'themes/balidemoapp/styles.css';
...
window.Vaadin['_vaadintheme_balidemoapp_globalCss'] = window.Vaadin['_vaadintheme_balidemoapp_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_balidemoapp_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_balidemoapp_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_balidemoapp_globalCss'].push(target);
  }
  if (!document['_vaadintheme_balidemoapp_componentCss']) {
    
    document['_vaadintheme_balidemoapp_componentCss'] = true;
  }
