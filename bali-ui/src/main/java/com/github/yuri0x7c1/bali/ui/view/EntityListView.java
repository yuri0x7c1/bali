...
public abstract class EntityListView<T> extends CommonView {
	...
	public EntityListView(Class<T> entityType, I18N i18n, EntityDataGrid<T> entityDataGrid) {
		...
		// create button
		createButton = new VButton(i18n.get("Create"), e -> create()).withVisible(false);

		// show button
		showButton = new VButton(i18n.get("Show"), e -> show()).withVisible(false);

		// edit button
		editButton = new VButton(i18n.get("Edit"), e -> edit()).withVisible(false);

		// delete button
		deleteButton = new DeleteButton()
			.withText("Delete")
			.withConfirmHandler(() -> delete())
			.withVisible(false);
